...
2. Explain: follow a representative trigger to its result. Add tables for repeated responsibilities/data and selected graph or state views for relationships that are hard to infer. Include conditions that affect the change. Connect technical names to what they do.
3. Inspect: open frozen source for a claim, then compare the complete diff. Keep the selected overview and core walkthrough visible. Collapse supplementary detail, not the entire explanation.
