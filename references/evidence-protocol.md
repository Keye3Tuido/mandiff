...
Secret redaction is the sole exception. The Agent identifies exact sensitive values in a private JSON file; `mandiff.py prepare --redactions` owns replacement, ephemeral HMAC key generation, original/display commitments, and private storage outside deliverables. `finalize` replaces the same values in semantic strings and acquired context, validates safe display bytes, records the evidence as `redacted`, and deletes script-owned private material. Use `mandiff.py cleanup` when abandoning a prepared redacted review. Never place a sensitive value directly on the command line or publish its plain hash.
